...
library(rpart) #tahmin edilen deDiEken faktC6r ya da nC<merik verilmeli. nC<merikse karar aDaC'larD1nD1 faktC6rse classification yapar

install.packages("rattle")
library(rattle) #gC6rselleEtirme iC'in

breast_cancer <- read.csv('C:/Users/Burak/OneDrive/Documents/Verisetleri/breast_cancer.csv' , header = TRUE , sep = "," , dec = ".")
View(breast_cancer)
...
### Karar ADğacının Görselleştirilmesi
...
confusionMatrix(pred_Entropy , testSet$diagnosis , mode = "prec_recall") # B=iyi huylu tümör için tahmin başarısı içi F1 skorları
...
#Hiperparametre tuning işlemi, bu ayarların farklı deDğerlerini deneyerek en iyi sonuçları veren kombinasyonu bulmaya çalışılır. 
...
modelLookup("rpart2") #rpart2'nin maxdepth'i tune ettiğini gösterir.
...
modelMD
#en yüksek accuracy için en yüksek doğruluDğ veren depth maxdepth olur.
...
modelRF$mtry # diğer parametreleri kontrol edebiliriz
# ayırma için 2 değişken kullacak sonucu çıktı
